...
void add_workout(struct workout **workouts, int *num_workouts) {
    ...
    *workouts = realloc(*workouts, *num_workouts * sizeof(struct workout));

    printf("\nEnter the name of the workout: ");
    getchar(); // Consume newline character
    fgets((*workouts)[*num_workouts - 1].name, 50, stdin);
    (*workouts)[*num_workouts - 1].name[strcspn((*workouts)[*num_workouts - 1].name, "\n")] = 0; // Remove newline character

    printf("Enter the number of reps: ");
    scanf("%d", &(*workouts)[*num_workouts - 1].reps);

    printf("Enter the number of sets: ");
    scanf("%d", &(*workouts)[*num_workouts - 1].sets);

    printf("Enter the time taken (in minutes): ");
    scanf("%f", &(*workouts)[*num_workouts - 1].time);
}
